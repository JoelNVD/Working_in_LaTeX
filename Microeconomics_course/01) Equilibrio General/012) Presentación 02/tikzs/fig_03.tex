\begin{tikzpicture}[scale=1.1]
	% Formación de la caja
		% Consumidor A
			...
			\draw[->] (0.5,0.5) -- (8.5,0.5) node[align=center, right] {\footnotesize $L^{B}$};
	...
	% Intersección de una dotación
		% Demanda: X
			\draw[dashed] (5.2,4) node[above] {\footnotesize $L^{B}$} -- (5.2,0.5) node[below]{\footnotesize $L^{A}$};
			\draw[dashed] (0.5,2) node[left] {\footnotesize $K^{A}$} -- (8,2)node[right]{\footnotesize $K^{B}$};
	...
	% Puntos
		\draw[black, fill=black] (5.2,2) circle[radius=0.05] node [above right, scale=0.25mm] {$(\overline{L}, \overline{K})$};
		%\draw[black, fill=black] (6,1.62) circle[radius=0.05] node [above right, scale=0.25mm] {$W$};
		
	% Flecha y rectángulo
		\draw [->] (2.48,3.29) -- (2.67,3.69) node [left, scale = 0.3mm] {\footnotesize $(w,r)$};
		\draw [rotate around={-25.33:(2.48,3.29)}] (2.48,3.29) rectangle (2.6, 3.4);
\end{tikzpicture}